...
import { CollectionsAuditPanel } from "@/components/admin/CollectionsAuditPanel";
import { useLanguage } from "@/hooks/useLanguage";

export const Route = createFileRoute("/admin/collections")({
  head: () => ({ meta: [{ title: "Collections audit — Waleed & Talaat" }] }),
  component: () => (
    <AdminGuard requireAdmin>
      <CollectionsAuditPage />
    </AdminGuard>
  ),
});

function CollectionsAuditPage() {
  const { t } = useLanguage();
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      <div className="surface-navy shadow-luxe flex flex-wrap items-center justify-between gap-4 rounded-3xl p-6">
        <div>
          ...
          <h1 className="text-2xl font-bold">{t("collections.auditTitle")}</h1>
        </div>
